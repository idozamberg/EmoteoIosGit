...
#endif
unichar ISO8601DefaultTimeSeparatorCharacters = DEFAULT_TIME_SEPARATOR;
...
@interface ISO8601DateFormatterHelper(UnparsingPrivate)
...
@implementation ISO8601DateFormatterHelper
...
- (id) init {
	if ((self = [super init])) {
		...
		timeSeparator = ISO8601DefaultTimeSeparatorCharacters;
		includeTime = NO;
		parsesStrictly = NO;
	}
	return self;
...
- (NSDateComponents *) dateComponentsFromString:(NSString *)string timeZone:(out NSTimeZone **)outTimeZone range:(out NSRange *)outRange {
	...
	if (strict) timeSep = ISO8601DefaultTimeSeparatorCharacters;
...
- (NSString *) weekDateStringForDate:(NSDate *)date timeZone:(NSTimeZone *)timeZone {
	...
	if(includeTime) {
		...
		unichar timeSep = self.timeSeparator;
		if (!timeSep) timeSep = ISO8601DefaultTimeSeparatorCharacters;
